...
if df_raw is not None and not df_raw.empty:
    ...
    with tab1:
        ...
        if 'Produto' in df.columns and 'CodProd' in df.columns:
            ...
            # NOVO: Adiciona a linha divisória com cor ESCURA para não sumir no fundo claro!
        ...
        if 'Categ' in df.columns and 'Produto' in df.columns:
            ...
            fig4 = px.treemap(df_tree,
                              ...
                              color='VlrVdacomImp', # Traz a barra de cor e o contraste de volta!
                              color_continuous_scale="Blues",
        ...
        if 'Categ' in df.columns:
            ...
            fig5 = px.bar(df_cat,
                          ...
                          color_discrete_sequence=['#3366CC'], # <-- COR SÓLIDA IGUAL AO GRÁFICO 1
                          custom_data=['Qtde', 'Perc_Fat', 'Perc_Qtd'])
    ...
    with tab3:
        ...
        for cod, dados_prod in df.groupby('CodProd'):
            ...
            # ==========================================
            # ---> NOVO: "PROCV" do Último Preço Pago 
            ...
            if dados_prod['DtPedido'].nunique() > 1:
                lista_produtos_ciclo.append({
                    ...
                    'Último Preço Pago': ultimo_preco, # ---> NOVO: Adicionado ao dicionário
        ...
        if not df_show.empty:
            ...
            st.dataframe(
                df_show.style.applymap(colorir_status, subset=['Status']).format({
                    ...
                    'Último Preço Pago': lambda x: format_brl(x) if x > 0 else '-', # ---> NOVO: Formatação R$
                    'Dias de Atraso': '{:.0f}',
                    'Ciclo (dias)': '{:.0f}',
                    'Cód. Últ. Pedido': '{:.0f}'
                }).hide(axis="index"),
